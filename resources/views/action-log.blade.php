...
@if ($paginator->total() === 0)
    <div class="text-center text-muted py-5">
        ...
        <p class="mt-3 mb-0">Действий пока нет.</p>
    </div>
@else
        ...
        @foreach ($paginator->items() as $row)
            ...
            <li class="tables-action-log__item @if ($isUndoEntry) tables-action-log__item--undo @endif">
                ...
                <div class="small mb-1">
                    ...
                    @elseif ($count !== null)
                        Объектов: <strong>{{ $count }}</strong>
                    @endif
                </div>
                @if ($payload !== [])
                    <div class="small mb-1">
                        Payload: <code class="tables-action-log__payload">{{ \Illuminate\Support\Str::limit(json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT), 300) }}</code>
                    </div>
                @endif
                @if ($result !== [])
                ...
                @if ($canUndo && $undoUrl !== null)
                    ...
                    >
                        ...
                        <span class="text-muted small ms-2">окно: {{ $windowMinutes }} мин</span>
                    </form>
                @elseif (! $isUndoEntry && $hasSnapshot && ! $withinWindow)
                    <div class="small text-muted mt-1">
                        <i class="bi bi-clock"></i> окно отката истекло
                    </div>
                @elseif (! $isUndoEntry && $hasSnapshot && $alreadyReverted)
                    <div class="small text-muted mt-1">
                        <i class="bi bi-check2-circle"></i> уже отменено
                    </div>
                @endif
            </li>
        @endforeach
    ...
    @if ($paginator->hasPages())
        <nav class="tables-action-log__pagination" aria-label="История">
