...
final class EloquentSource implements Source
{
    ...
    private function applyConditions(Builder $query, Query $q): void
    {
        ...
        foreach ($q->conditions as $cond) {
            ...
            self::applyBuiltin($query, $cond->field, $cond->operator, $cond->value);
        }
    }
    ...
    /**
     * Built-in применение FilterCondition без Field-aware кастомизаций
     * (fallback для случаев, когда $resource === null или Field не объявлен).
     * Копия логики устаревшего {@see FilterApplier::applyBuiltin()}.
     *
